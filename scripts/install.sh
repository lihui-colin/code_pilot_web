...
Options:
    --host <host-address>: browser-reachable host machine address, not container IP (required)
    --service-port <port>: CodePilot Web HTTPS port (required)
...
if [[ "$non_interactive" == false ]]; then
    [[ -n "$host" ]] || read -r -p "Browser-reachable host machine IP or hostname (not the container IP): " host
    [[ -n "$service_port" ]] || read -r -p "CodePilot Web HTTPS port: " service_port
...
[[ -n "$host" ]] || die "--host is required and must be the browser-reachable host machine address"
[[ -n "$service_port" ]] || die "--service-port is required"
[[ "$host" != "0.0.0.0" && "$host" != "::" && "$host" != "[::]" ]] \
|| die "--host must be the host machine IP or hostname used by browsers, not a wildcard or container-only address"
...
ports=("$service_port" "$zellij_port" "$viewer_port" "$openvscode_port")
