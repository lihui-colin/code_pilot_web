...
restart_delay_ms="${TERMINAL_WEB_RESTART_DELAY_MS:-0}"
...
if [[ ! "$restart_delay_ms" =~ ^[0-9]+$ || "$restart_delay_ms" -gt 10000 ]]; then
    echo "TERMINAL_WEB_RESTART_DELAY_MS must be an integer from 0 to 10000" >&2
    exit 2
fi
...
echo "[$(date -Iseconds)] Terminal Web support services restarted"
