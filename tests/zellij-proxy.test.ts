...
describe('Zellij Web same-origin proxy', () => {
  ...
  it('rewrites the HTML base and proxies HTTP and WebSocket paths', async () => {
    ...
    try {
      ...
      expect(toolbar?.style.getPropertyValue('--shortcut-1-x')).toBe('67.88px');
      expect(toolbar?.style.getPropertyValue('--shortcut-1-y')).toBe('-67.88px');
      expect(toolbar?.style.getPropertyValue('--shortcut-2-x')).toBe('92.73px');
      expect(toolbar?.style.getPropertyValue('--shortcut-2-y')).toBe('-24.85px');
      expect(toolbar?.style.getPropertyValue('--shortcut-3-x')).toBe('92.73px');
      expect(toolbar?.style.getPropertyValue('--shortcut-3-y')).toBe('24.85px');
      expect(toolbar?.style.getPropertyValue('--shortcut-4-x')).toBe('67.88px');
      expect(toolbar?.style.getPropertyValue('--shortcut-4-y')).toBe('67.88px');
      const savedPosition = JSON.parse(dom.window.localStorage.getItem('codepilot-zellij-shortcuts-position') || '{}') as { side: string; topRatio: number };
      ...
      expect(Number.parseFloat(toolbar?.style.top || '')).toBeCloseTo(8 + savedPosition.topRatio * (1000 - 45 - 16), 5);
      ...
      expect(toolbar?.style.getPropertyValue('--shortcut-1-x')).toBe('96px');
      expect(toolbar?.style.getPropertyValue('--shortcut-1-y')).toBe('0px');
      expect(toolbar?.style.getPropertyValue('--shortcut-4-x')).toBe('0px');
      expect(toolbar?.style.getPropertyValue('--shortcut-4-y')).toBe('96px');
      dom.window.close();
