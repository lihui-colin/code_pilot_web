...
describe('Zellij Web same-origin proxy', () => {
  ...
  it('rewrites the HTML base and proxies HTTP and WebSocket paths', async () => {
    ...
    try {
      ...
      expect(shortcutScript).toContain("toolbar.style.setProperty('--shortcut-toggle-font-size', 1.15 * toolbarScale + 'rem')");
      expect(shortcutScript).toContain("toolbar.style.setProperty('--shortcut-hint-font-size', .58 * toolbarScale + 'rem')");
      expect(shortcutScript).toContain("toolbar.style.setProperty('--shortcut-hint-gap', .18 * toolbarScale + 'rem')");
      expect(shortcutScript).toContain("const mobileWidthStorageKey = 'codepilot-zellij-shortcuts-mobile-width'");
      expect(shortcutScript).toContain('const desktopLikeTouch = touchDevice');
      expect(shortcutScript).toContain('const savedViewportRatio = hasSavedMobileWidth');
      expect(shortcutScript).toContain('Math.max(savedViewportRatio, desktopLikeTouch');
      ...
      Object.assign(dom.window, { __zjImeBypass: { sendFn: (sequence: string) => sentSequences.push(sequence) }, term: terminal });
      ...
      interrupt?.click();
      expect(sentSequences).toEqual(['\x10n', '\x03']);
      clickToggle();
      ...
      arrowUp?.click();
      arrowDown?.click();
      expect(sentSequences).toEqual(['\x10n', '\x03', '\t', '\t', '\x1b[A', '\x1b[B']);
      ...
      expect(toolbar?.style.getPropertyValue('--shortcut-font-size')).toBe('1.9500000000000002rem');
      expect(toolbar?.style.getPropertyValue('--shortcut-toggle-font-size')).toBe('2.875rem');
      expect(toolbar?.style.getPropertyValue('--shortcut-hint-font-size')).toBe('1.45rem');
      expect(toolbar?.style.getPropertyValue('--shortcut-hint-gap')).toBe('0.44999999999999996rem');
      expect(toolbar?.style.getPropertyValue('--shortcut-1-x')).toBe('32.56px');
      expect(terminal.options.fontSize).toBe(37.5);
      dom.window.localStorage.removeItem('codepilot-zellij-shortcuts-mobile-width');
