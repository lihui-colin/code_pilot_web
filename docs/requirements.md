...
页面同时提供类似 VSCode “Open Folder”的“添加文件夹”操作。用户可以从服务器文件系统根目录逐层进入服务进程可读的目录，也可以输入初始目录；已记忆的初始目录在下次打开选择器时自动填入并立即前往。点击目录树进入子目录或点击“上一级”时，初始目录输入框必须同步为当前浏览目录。初始目录为空时，选择器改为记忆最后一次成功选择的目录，并在下次打开时直接返回该目录。目录记忆只保存在当前浏览器中，后续目录浏览只交换后端签发的不透明目录 ID。当前目录或任意可读子目录均可选择；选中的 workspace 外目录持久化到状态文件，服务重启后继续显示，并可从列表移除；移除不会删除服务器文件。
...
每个 Git repository 条目提供“与 Codex 对话”链接，在新标签页打开独立的 `/codex-chat` 页面；当该 repository 在后台生成时，管理首页仍保持“与 Codex 对话”单行文字，并通过动态颜色变化提示生成状态。页面加载时先检查后台服务能否调用 Codex CLI；可用时展示版本并启用输入，不可用时禁用发送并提示检查安装、可执行权限和后台服务用户的 `PATH`。用户发送的消息和头像靠右显示，Codex 回复和头像靠左显示；用户显示名默认为 `me`，可以在抽屉面板中修改并保存在当前浏览器。任意 Codex 页面都可以在抽屉的字体下拉列表中选择常用字体，并修改 `12` 到 `24` 像素的字号；偏好在当前浏览器的所有 Codex 页面间共享，并可以恢复为服务器配置的默认值。“新对话”、repository 路径、返回首页和运行信息放在默认隐藏的抽屉面板中，通过顶部菜单按钮打开，并支持遮罩和 Esc 关闭；主对话区域始终占满可用宽度。消息输入框提供“Add file”，从服务端列出的当前 repository 普通文本文件中最多选择 8 个作为本次消息的限定上下文；已选文件以可移除标签展示，发送后也显示在用户消息中。页面展示后台响应状态和停止操作；发送后至 Codex app-server 握手完成期间显示“正在启动 Codex…”状态，并在对应回复前按事件顺序显示脱敏的思考摘要、命令执行、文件修改和工具调用卡片。等待首段回复时显示等待动画，收到部分助手文本但 turn 尚未结束时在最新回复下持续显示动态“正在继续生成”提示，完成后自动隐藏；失败或停止且没有回复时不显示等待动画。用户停留在消息底部时流式内容自动跟随；向上滚动阅读历史后不得被新片段强制拉回，并提供回到最新消息的快捷入口。消息输入区位于消息滚动区域末尾并随消息一起滚动，不得固定在视口底部。关闭或刷新页面时未完成的 Codex turn 继续在管理服务后台运行，再次进入同一 repository 时恢复消息、conversation ID 和运行状态。换浏览器、换设备或管理服务重启后，服务端必须使用已保存的 conversation ID 从 Codex thread 恢复历史；浏览器快照只能作为服务端历史暂时不可用时的兼容回退，不能作为跨浏览器恢复的唯一来源，并通过 Codex 原生 resume 继续会话。支持 Enter 发送、Shift+Enter 换行，并适配桌面和移动浏览器。

握手完成后输入框继续可用，用户可通过“追加输入”把新的自然语言消息发送给当前运行中的 turn。活动卡片显示 Codex 对外提供的 detailed 思考摘要、实际命令文本与执行状态；不展示 private reasoning content 或命令输出。
...
Codex 对话快照只包含 conversation ID、用户与助手文本、脱敏活动摘要、运行状态和脱敏错误。活动摘要可包含 Codex 对外提供的 detailed reasoning summary、实际命令文本、工具名称、执行状态和 repository 内相对文件路径，但不得包含 private reasoning content、命令输出、工具参数或结果、diff 正文、标准错误、附件内容或异常堆栈；当前 repository 绝对路径必须替换为 `.`。

## 管理页面

第一屏直接展示可操作界面：

- “系统状态”入口；管理服务、系统就绪状态、Zellij、code-viewer、OpenVSCode 和 Codex CLI 等运行组件状态在该页面集中展示。
...
部分工具未就绪时，页面仍允许从 Workspace 区域打开能够读取的 Zellij Session 列表和系统设置面板；未就绪状态只阻止依赖缺失工具的操作。
...
- 服务器绝对路径。
- 除主页默认隐藏的系统设置面板之外的 Zellij Token。
- 环境变量。
- 原始外部命令输出。
- 未经处理的异常堆栈。
...
- Zellij Web 保留自身 Token 验证；管理服务首次启动创建专用 Token，同时保存名称和值，并在主页默认隐藏的系统设置面板中提供复制、删除和重新创建操作。系统设置通过明确的设置按钮打开，可通过关闭按钮、遮罩和 Esc 关闭；后台服务重启操作也收纳在该面板中。
