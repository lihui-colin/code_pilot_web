...
页面同时提供类似 VSCode “Open Folder”的“添加文件夹”操作。用户可以从服务器文件系统根目录逐层进入服务进程可读的目录，也可以输入不带前导 `/` 且不含 `..` 路径段的相对初始目录；后续目录浏览只交换后端签发的不透明目录 ID。当前目录或任意可读子目录均可选择；选中的 workspace 外目录持久化到状态文件，服务重启后继续显示，并可从列表移除；移除不会删除服务器文件。
