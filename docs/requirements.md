...
消息输入框默认显示一行文字高度，内容变长或通过 Shift+Enter 换行时随内容自动增高，达到最大高度后在输入框内部滚动。用户只提交 repository ID、自然语言消息和服务端签发的上下文文件 ID。服务端重新解析 repository 和每个文件的真实路径并执行对应来源的路径边界校验，然后在该 repository 中使用服务端固定的 `--yolo` 参数运行 Codex CLI。`--yolo` 会跳过审批并绕过 Codex 沙箱，因此该入口只适用于受信任的 VPN 用户。用户不能从浏览器提交目录、绝对路径、文件路径、可执行文件、命令参数、环境变量或 Codex 配置。
