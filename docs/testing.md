...
12. 首次消息固定启动 `codex app-server --listen stdio://`，完成 `initialize`/`initialized` 后调用 `thread/start` 和 `turn/start`；`turn/start` 固定请求 `summary: "detailed"`，包含校验后文件 JSON 的 prompt 只通过 JSON-RPC 文本输入发送，且 `shell: false`。
...
16. 消息接口返回 `202` 和后台对话快照；快照查询返回消息、脱敏活动摘要、conversation ID、状态、脱敏错误和更新时间。
17. 后端把 detailed reasoning summary、实际命令文本与状态、文件修改相对路径和工具通用名称转换为活动摘要；当前 repository 绝对路径替换为 `.`。不返回 private reasoning content、命令输出、工具参数或结果、diff 正文、stderr、usage、异常堆栈或原始失败详情。
...
22. 再次进入同一 repository 时恢复服务端快照；同源 SSE 在连接或重连时发送 `conversation.snapshot`，运行中发送 `turn.started`、`thread.started`、`turn.steered`、`app-server.event`、`activity.updated`、`message.delta`、`message.completed`、`turn.completed`，清空时发送 `conversation.cleared`，且 SSE `event` 与 JSON `data.type` 一致。每条 app-server response、notification 和 server request 都产生按序递增的安全元数据事件，不包含原始 params/result/error/payload。密集 delta 可在不超过 40 毫秒的窗口内合并，浏览器按序归并事件并用完成事件校正最终助手文本、活动状态和 turn 状态，断线重连后恢复完整助手文本及脱敏活动；活动卡片显示在所属回复正文之前。浏览器对运行中快照的持久化不超过每 100 毫秒一次，最终状态立即保存；首段文本到达前显示等待动画，部分文本到达后最新助手消息持续显示动态生成提示，完成后提示消失；停留底部时自动跟随流式输出，向上阅读历史时保持当前滚动位置并提供回到最新消息入口。输入区处于消息滚动区域末尾，向上滚动消息时必须离开视口而不是固定在底部。
23. 运行中的输入只能调用固定 steer endpoint，请求体严格为 `{ "message": string }`，并由后端转换为包含服务端 `threadId`、`expectedTurnId` 和 text input 的 `turn/steer`；Origin、repository、额外字段和非运行状态都有聚焦测试，前端不能提交任意 JSON-RPC 方法或参数。
...
26. 管理服务重启、换浏览器或换设备后，从状态文件恢复 conversation ID，并通过 app-server `thread/read`（`includeTurns: true`）恢复脱敏的历史用户/助手消息和受支持活动摘要；页面无需依赖原浏览器缓存即可显示完整历史，下一条消息固定使用 app-server `thread/resume` 恢复该 ID。thread 历史读取失败、thread ID 与 repository cwd 不匹配或历史超限时返回脱敏错误，不得静默创建新 conversation。
...
29. `thread/resume` 返回 active-writer 冲突时分类为 `CODEX_CONVERSATION_IN_USE`，页面显示关闭其他客户端或新建对话的明确提示；快照不留下本次失败的用户消息、助手占位或活动，且不暴露 thread ID 或原始 app-server 错误。非 `thread/resume` 错误仍保持通用脱敏分类。
