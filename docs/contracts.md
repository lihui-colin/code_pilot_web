...
移动触摸设备纵向滑动终端时，注入脚本不得在 `document` 捕获阶段拦截或阻止触摸事件，也不得自动发送 `Ctrl+T`、`PageUp` 或 `PageDown`。Zellij Web `0.44.3` 自带的终端触摸处理负责把纵向滑动转换为鼠标滚轮报告，使普通手机模式与浏览器“电脑版”模式使用一致的终端滚动路径；浮动快捷球的拖动手势仍只由快捷球自身处理。
