...
- 固定同源重启接口与统一服务脚本，安全清理全部托管后台进程，并只重新拉起管理服务与 Zellij Web。
...
- 固定 `codex app-server --listen stdio://` 进程和 `initialize`、`thread/start|resume`、`turn/start` JSON-RPC 调用，以及包含服务端校验文件上下文的 prompt。
- 服务端 repository 级对话快照、conversation ID 归属校验和同一 repository 并发保护。
- 助手文本从 `item/agentMessage/delta` 增量解析到快照，通过同源 SSE 实时推送，并执行绝对路径替换和错误脱敏。
- 页面重进恢复、运行中 SSE 更新、浏览器安全快照和 Codex 原生 resume。
